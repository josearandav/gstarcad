...
<head>
    ...
    <meta property="og:site_name" content="WPS Office Blog"/>
...
<div class="placebox">
    <div class="maincon">
        <ul>
            ...
            <li><a href="<base:contextpath/>/news/"><em class="c2"></em>News</a></li>
        </ul>
...
<div class="news_xiang maincon or">
    ...
    <div class="news_xiang_r">
        <div class="top or">
            ...
            <p>
                <span>
                	<i class="b1"></i>Date: <fmt:formatDate value="${news.releaseTime}" pattern="yyyy-MM-dd"/>
                </span>
                <span><i class="b2"></i>${news.views}</span>
                <span>
                	<i class="b3"></i>Tag:
					<c:if test="${!empty news.tags }">
                        <c:set value="${ fn:split(news.tags, '#') }" var="tags"/>
                        <c:forEach items="${tags }" var="newsTag">
                            ${newsTag}&nbsp;&nbsp;
                        </c:forEach>
                    </c:if>
                </span>
        ...
        <div class="bottom or">
            <a class="prev"
               href="<base:contextpath/>/news/${lastNews.getNewsTitle()}?id=${lastNews.id}">${lastNews.title}</a>
            <a class="next"
               href="<base:contextpath/>/news/${nextNews.getNewsTitle()}?id=${nextNews.id}">${nextNews.title}</a>
        </div>
...
<div class="maincon">
    <div class="recent-hotnews">
        <ul class="clearfix">
            <c:forEach items="${relatedNews}" var="it">
                <li>
                    <a href="<base:contextpath/>/news/${it.newsTitle}?id=${it.id}">
                        <div class="shorticon">
